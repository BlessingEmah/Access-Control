...
describe("Access Control", async() => {
 ...
 beforeEach(async function () {
  ...
  const AgreedPrice = await ethers.getContractFactory("AgreedPrices", deployer);
...
//     //@ts-ignore
//    await expect(this.agreedPrice.connect(attacker).updatePrice(2000)).revertedWith("Ownable: caller is not the owner");
//   });
...
//       await expect(this.agreedPrice.connect(attacker).changeOwner(attacker.address)).to.be.revertedWith("Ownable: caller is not the owner");
//     }); 
 })
});